...
<body>
    <div class="dashboard-container">
        ...
        if(isset($_GET['modifyArticle'])){
            $article = $_GET['modifyArticle'];
            $query = mysqli_query($con,"SELECT article_title , article_content FROM articles where articleID = $article");
                 ...
                 if($result = mysqli_fetch_assoc($query)){
                    ...
                    <form action='' method='POST'>
                      <label>Title:</label>
                      <input type='text' id='title' name='title' value='{$result['article_title']}' required>
  
                      <label>Content:</label>
                      <textarea id='content' name='content' rows='5' required>{$result['article_content']}</textarea>
  
                      <button type='modify-article' name='modify-article'>Modify Article</button>
                  </form>";
                  if (isset($_POST['modify-article'])) {
                      ...
                      if (mysqli_query($con, "UPDATE articles set article_title ='$title'  , article_content = '$content' WHERE articleID = $article")) {
           ...
           <h2>Add New Article</h2> 
                    ...
                    <tbody>
                        ...
                        while ($result = mysqli_fetch_assoc($articles)) {
                            echo "<tr>
                                    ...
                                    <td>
                                        <a class='edit-btn' href='dashboard.php?modifyArticle={$result['articleID']}'><button class='edit-btn'>Edit</button></a>
            ...
            <section class="manage-comments container" style="display: none;">
                ...
                <table>
                    ...
                    <tbody>
                        <?php
                        $authID = $_SESSION['id'];
                        $comments = mysqli_query($con, "SELECT ar.article_title , com.visitor_name , com.comment_content , com.commentID
                                                        FROM comments com
                                                        JOIN articles  ar ON ar.articleID = com.articleID
                                                        WHERE ar.authorID = $authID
                                                        ");
            ...
            <section class="stats container" style="display: none;">
                ...
                if (isset($_SESSION['id'])) {
                    $authorID = $_SESSION['id'];
                    $query = mysqli_query($con, "SELECT sum(stat.views) as views , sum(stat.likes) as likes , COUNT(com.commentID) as comment_num , COUNT(ar.articleID) as article_num , ar.authorID , aut.author_name
                                                FROM   articles ar
                                                join authors aut on aut.authorID = ar.authorID
                                                join comments com on ar.articleID = com.articleID
                                                join statistiques stat on ar.articleID = stat.articleID
                                                WHERE ar.authorID = $authorID;
                                        ");
                    if ($stats = mysqli_fetch_assoc($query)) {
                        echo " <div class='stat-card'>
                    <h3>Total Articles</h3>
                    <p>{$stats['article_num']}</p>
                </div>
                <div class='stat-card'>
                    <h3>Views</h3>
                    <p>" . ($stats['views'] ??  0) . "</p>
                ...
                <div class='stat-card'>
                    <h3>Comments</h3>
                    <p>{$stats['comment_num']}</p>
                </div>";
                    }
                }
                ?>
            </section>
            <section class="modify-article container" style="display: none;" >
                <h2>Modify Article</h2>
